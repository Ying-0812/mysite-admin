...
<script>
import { getReviews, delReview } from "@/api/reviews";
...
export default {
  ...
  created() {
    this.getReviewList(this.nowCurrent, this.pageSize);
  },
  methods: {
    pageSizeChange(num) {
      this.nowCurrent = 1
      this.pageSize = num
      this.getReviewList(this.nowCurrent,num);
    },
    currentChange(page) {
      this.nowCurrent = page
      this.getReviewList(page,this.pageSize);
    },
    getReviewList(page, limit) {
      this.loading = true;
      this.data = [];
      getReviews(page, limit).then((res) => {
        this.loading = false;
    ...
    delReviews(data){
      this.$confirm("确定删除该评论吗?", "提示", {
        ...
        .then(() => {
          delReview(data._id).then(() => {
            this.$message({
              type: "success",
              message: "删除成功!",
            });
            this.getReviewList(this.nowCurrent = 1, this.pageSize);;
          });
        })
        .catch(() => {
          this.$message({
